...
async function nsRequest(method, path, data = null) {
  ...
  const config = {
    method, url,
    headers: { 'Authorization': getTBAHeader(method, url), 'Content-Type': 'application/json' }
  };
...
async function suiteQL(query) {
  ...
  try {
    const response = await axios.post(url, { q: query }, {
      headers: { 'Authorization': getTBAHeader('POST', url), 'Content-Type': 'application/json', 'Prefer': 'transient' }
    });
...
async function getFulfilledShipments(since) {
  const sinceDate = since.split('T')[0];
  const rows = await suiteQL(
    `SELECT f.id, so.custbody_tiktok_order_id FROM itemfulfillment f ` +
    `INNER JOIN transaction so ON so.id = f.createdfrom ` +
    `WHERE f.lastmodifieddate >= TO_DATE('${sinceDate}', 'YYYY-MM-DD') AND f.status = 'C' ` +
    `AND so.custbody_tiktok_order_id IS NOT NULL`
  );
  ...
  for (let i = 0; i < ifIds.length; i += 50) {
    ...
    for (const p of pkgs) {
      if (!trackingMap[p.itemfulfillment]) trackingMap[p.itemfulfillment] = { tracking: p.trackingnumber || '', carrier: p.carrierpackaging || '' };
    }
  }

  return rows.map(r => ({
    id: r.id,
    tiktokOrderId: r.custbody_tiktok_order_id,
    trackingNumber: trackingMap[r.id]?.tracking || '',
    carrier: trackingMap[r.id]?.carrier || ''
  }));
}

module.exports = { nsRequest, suiteQL, createSalesOrder, getCustomerByEmail, getItemIdBySku, createInventoryItem, getFulfilledShipments };
