...
const omegaman = (function ($, window, document) {
	...
	function largerScreenFunctionality() {
		...
		parentLinks.forEach(function (parentLink) {
			parentLink.addEventListener('mouseenter', function (event) {
				/* sd2: .hoverer's mouseover (below) closes the whole mega-menu on
				   any mouse movement outside the active panel — including the gap
				   between two parent tabs (e.g. Powerstroke -> Duramax), forcing
				   shoppers to close and reopen the menu instead of just sliding
				   across. Flagging the moment a sibling parent link re-activates
				   lets the hoverer handler below ignore that one closing signal. */
				window.sd2OmegaSwitching = true;
	...
	hoverer.addEventListener('mouseover', function(event) {
		...
		/* sd2: if this mouseover was caused by crossing into another parent
		   tab's own mouseenter (see parentLinks loop above), that link's
		   handler already swapped the active panel — don't immediately close
		   what it just opened. */
		if (window.sd2OmegaSwitching) {
			window.sd2OmegaSwitching = false;
			return;
		}
